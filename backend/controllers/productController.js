...
//         deducted_price: req.body.deducted_price,
...
// Person.update({'items.id': 2}, {'$set': {'items.$.name': 'updated item2','items.$.value': 'two updated' }})
// ~~ Better example below ~~
// model.update({ _id: 1, "items.id": "2" },{$set: {"items.$.name": "yourValue","items.$.value": "yourvalue",}})



// const list_element = await Product.find({_id: req.body.product}, {rated_users: { $elemMatch : {user: req.user.id}}})
// db.articles.find({ _id : 3 },{ stock : { $elemMatch : { country : "01" } } })
...
// User.findOne({id: req.body.myId}).select({ Friends: {$elemMatch: {id: req.body.id}}}),
        ...
        // for (let item of product.rated_users) {
        //     if (item.user==user.id) {
        //         product.total_rate_value = product.total_rate_value - item.rate_value + rate_value
        ...
        // const user_rated_before = product.rated_users.some( async (item) => {
        //     if (item.user==user.id) {

        //         product.total_rate_value -= item.rate_value
        //         product.rated_users.pull({_id: item._id})
...
//         const { name, product_info, phone, delivery_address, paymentmethod} = req.body

//         if (!product_info || !name || !phone || !delivery_address || !paymentmethod) {
//             res.json({message: "required fields missing"})
//             return
//         }

//         // The code below checks if the ordered products are available in stock
//         //
//         // out_of_stock = []
//         // product_info.forEach( 
//         //     (item) => {
//         //         item_product = await Product.find(item.product, 'name stock')
//         //         if (item_product.stock < item.quantity) {
//         //             out_of_stock.push(item_product)
//         //         }
//         //     }
//         // )
//         // if (out_of_stock) {
//         //     res.json({message: `some products are out of stock: ${out_of_stock}`})
//         //     return
//         // }
...
//             const user = await User.findById(decoded.id, 'email')
//             user_id = user.id
...
//             await Order.create({product_info, ordered_by: user_id, name, phone, delivery_address, payment_method: paymentmethod})
//             res.status(201).json({message: "order creation successful"})
...
//             if (paid) {
//                 await Order.create({product_info, ordered_by: user_id, name, phone, delivery_address, payment_method: paymentmethod, payment_status: 'paid'})
//                 res.status(201).json({message: "order creation successful"})
//             } else {
//                 res.json({message: 'payment not received'})
//             }
//         } else {
//             res.json({message: 'payment method incorrect'})
//         }
//     } catch (error) {
